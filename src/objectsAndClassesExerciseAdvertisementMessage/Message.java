...
public class Message {
    ...
    public String randomMessage() {
        ...
        return String.format("%s %s %s – %s.", phrases[random.nextInt(phrases.length)], events[random.nextInt(events.length)], author[random.nextInt(author.length)], town[random.nextInt(town.length)]);
    }
}
